...
import org.testany.fakerpp.core.util.earclipping.Point ;
import org.geojson.*;
...
public class GeoBase {
    ...
    public List<List<Point>> points(String origin) {
        switch (input) {
            case "wkt":
     ...
     * @return
     */
    public List<List<Point>> wkt2Points(String wkt) {
        String[] latLngs = wkt
                .trim().split(",");
        ...
        for (String latLng : latLngs) {
            ...
            points.add(new Point(latLngDouble[0], latLngDouble[1]));
        }
     ...
     * @return
     */
    public List<List<Point>> geojson2Points(String geojson) {
        try {
            ...
            for (Feature feature : fc.getFeatures()) {
                ...
                for (LngLatAlt lngLat : polygon.getExteriorRing()) {
                    polygonPoints.add(new Point(lngLat.getLatitude(), lngLat.getLongitude()));
                }
     ...
     */
    public List<Point> checkEnd2End(List<Point> origin){

        Point start = origin.get(0);
        Point end = origin.get(origin.size() - 1);
