package org.testd.ui.fxweaver.core;
...
public class FxWeaver {
    ...
    public <C> C loadControl(Class<C> controllerClass) {
        String fxmlLocation = buildFxmlReference(controllerClass);
        URL url = controllerClass.getResource(fxmlLocation);
        try (InputStream fxmlStream = url.openStream()) {
            LOG.debug("Loading FXML resource at {}", url);
            FXMLLoader loader = new FXMLLoader();
            C bean = controllerClass.cast(beanFactory.call(controllerClass));
            loader.setRoot(bean);
